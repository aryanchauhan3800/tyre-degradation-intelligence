...
export class RaceCarScene {
  ...
  private loadBlenderGlb(): void {
    ...
    loader.load(
      '/models/f1_car.glb',
      (gltf) => {
        this.setupBlenderModel(gltf.scene, 'BLENDER_F1_CAR', 0.24);
      },
      undefined,
      (err) => {
        console.warn('Could not load f1_car.glb, falling back to f2_car.glb:', err);
        loader.load(
          '/models/f2_car.glb',
          (gltf2) => {
            this.setupBlenderModel(gltf2.scene, 'BLENDER_F2_CAR', 0.44);
          },
          undefined,
          (err2) => {
            console.warn('Fallback to procedural race car geometry:', err2);
          }
        );
      }
    );
  ...
  private setupBlenderModel(model: THREE.Group, name: string, scale: number): void {
    model.name = name;
    // Align with Three.js car heading (+Z forward)
  ...
  public updateTelemetry(
    ...
    for (const corner of corners) {
      ...
      if (bMats && bMats.length > 0) {
        for (const bMat of bMats) {
          const cornerTdi = fourWheelStates && fourWheelStates[corner]?.tdi;
          if (cornerTdi !== null && cornerTdi !== undefined) {
            this.applyThermalColor(bMat, cornerTdi);
          } else if (this.selectedCorner === corner) {
            bMat.emissive.setHex(0x00f0ff);
            bMat.emissiveIntensity = 0.45;
  ...
  private applyThermalColor(mat: THREE.MeshStandardMaterial, tdi: number): void {
    // 0-15: Cold / Fresh compound (Electric Blue)
    // 16-35: Optimal operating window (Racing Green)
    // 36-55: Moderate wear warning (Thermal Yellow)
    // 56-75: High wear / heat stress (Hot Orange)
    // 76-100: Critical degradation (Glowing Fiery Red)
    if (tdi <= 15) {
      mat.color.setHex(0x102040);
      mat.emissive.setHex(0x2563eb); // Blue
      mat.emissiveIntensity = 0.3 + (tdi / 15.0) * 0.3;
    } else if (tdi <= 35) {
      mat.color.setHex(0x10301c);
      mat.emissive.setHex(0x10b981); // Green
      mat.emissiveIntensity = 0.5 + ((tdi - 15) / 20.0) * 0.3;
    } else if (tdi <= 55) {
      mat.color.setHex(0x352e05);
      mat.emissive.setHex(0xeab308); // Yellow
      mat.emissiveIntensity = 0.8 + ((tdi - 35) / 20.0) * 0.4;
    } else if (tdi <= 75) {
      mat.color.setHex(0x401c05);
      mat.emissive.setHex(0xf97316); // Orange
      mat.emissiveIntensity = 1.2 + ((tdi - 55) / 20.0) * 0.5;
    } else {
      mat.color.setHex(0x4a0808);
      mat.emissive.setHex(0xef4444); // Red
      mat.emissiveIntensity = 1.6 + Math.min(0.6, ((tdi - 75) / 25.0) * 0.6);
    }
  }
